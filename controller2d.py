...
class Controller2D(object):
    ...
    def get_lookahead_index(self, lookahead_distance):
        ...
        for i in range(len(self._waypoints)):
            dist = np.linalg.norm(np.array([np.subtract(self._waypoints[i][0] , self._current_x),np.subtract(self._waypoints[i][1] , self._current_y)]))
        ...
        for i in range(min_idx + 1, len(self._waypoints)):
            ...
            total_dist += np.linalg.norm(np.array([
                    np.subtract(self._waypoints[i][0] , self._waypoints[i-1][0]),
                    np.subtract(self._waypoints[i][1] , self._waypoints[i-1][1])]))
            lookahead_idx = i
    ...
    def update_desired_speed(self):
        ...
        for i in range(len(self._waypoints)):
            dist = np.linalg.norm(np.array([
                    np.subtract(self._waypoints[i][0] , self._current_x),
                    np.subtract(self._waypoints[i][1] , self._current_y)]))
            if dist < min_dist:
                min_dist = dist
                min_idx = i
    ...
    def set_steer(self, input_steer_in_rad):
        ...
        input_steer = np.multiply(self._conv_rad_to_steer , input_steer_in_rad)
