...
class BehaviouralPlanner:
    def __init__(self, lookahead, stopsign_fences, lead_vehicle_lookahead,waypoints):
    ...
    def transition_state(self, ego_state, closed_loop_speed):
        """Handles state transitions and computes the goal state.  
        
        args:
            self.waypoints: current self.waypoints to track (global frame). 
        ...
        variables to set:
            self._goal_index: Goal index for the vehicle to reach
                i.e. self.waypoints[self._goal_index] gives the goal waypoint
            self._goal_state: Goal state for the vehicle to reach (global frame)
                format: [x_goal, y_goal, v_goal]
            self._state: The current state of the vehicle.
                available states: 
                    FOLLOW_LANE         : Follow the global self.waypoints (lane).
        ...
        if self._state == FOLLOW_LANE:
            ...
            closest_len, closest_index = get_closest_index(self.waypoints, ego_state)
            # ------------------------------------------------------------------

            # Next, find the goal index that lies within the lookahead distance
            # along the self.waypoints.
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            # goal_index = ...
            goal_index = self.get_goal_index(ego_state, closest_len, closest_index)
            while self.waypoints[goal_index][2] <= 0.1: goal_index += 1
            # ------------------------------------------------------------------
            ...
            # self._goal_index = ...
            # self._goal_state = ...
            goal_index, stop_sign_found = self.check_for_stop_signs(closest_index, goal_index)
            self._goal_index = goal_index
            self._goal_state = self.waypoints[goal_index]
            # ------------------------------------------------------------------
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                #_, stop_sign_found = self.check_for_stop_signs(self.waypoints, closest_index, goal_index)
    ...
    ######################################################
    ######################################################
    # Gets the goal index in the list of self.waypoints, based on the lookahead and
    ...
    def get_goal_index(self, ego_state, closest_len, closest_index):
        ...
        args:
            ...
            closest_index: index of the waypoint which is closest to the vehicle.
                i.e. self.waypoints[closest_index] gives the waypoint closest to the vehicle.
        returns:
            wp_index: Goal index for the vehicle to reach
                i.e. self.waypoints[wp_index] gives the goal waypoint
        """
        ...
        # the planner.  No need to check additional self.waypoints.
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        # while wp_index < len(self.waypoints) - 1:
        #   arc_length += ...
        while wp_index < len(self.waypoints) - 1:
            arc_length += np.sqrt((self.waypoints[wp_index][0] - self.waypoints[wp_index+1][0])**2 + (self.waypoints[wp_index][1] - self.waypoints[wp_index+1][1])**2)
    ...
    def check_for_stop_signs(self, closest_index, goal_index):
        ...
        args:
            self.waypoints: current self.waypoints to track. (global frame)
                ...
                goal_index (current): Current goal index for the vehicle to reach
                    i.e. self.waypoints[goal_index] gives the goal waypoint
        variables to set:
        ...
        for i in range(closest_index, goal_index):
            ...
            for key,stopsign_fence in enumerate(self._stopsign_fences):
                ...
                wp_1   = np.array(self.waypoints[i][0:2])
                wp_2   = np.array(self.waypoints[i+1][0:2])
...
def get_closest_index(waypoints, ego_state):
    """Gets closest index a given list of self.waypoints to the vehicle position.
    args:
        self.waypoints: current self.waypoints to track. (global frame)
            length and speed in m and m/s.
            (includes speed to track at each x,y location.)
