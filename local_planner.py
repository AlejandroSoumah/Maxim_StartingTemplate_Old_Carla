...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, ego_state):
        ...
        # ------------------------------------------------------------------
        #goal index is a number goal_state_set[][goal_index] would give us the current goal_state_set
        # waypoints[goal_index] --> goal waypoint 
        if goal_index < len(self.waypoints) -1:
            delta_x = np.subtract(self.waypoints[goal_index + 1][0] , self.waypoints[goal_index][0])
            delta_y = np.subtract(self.waypoints[goal_index + 1][1] , self.waypoints[goal_index][1])
        else:
            delta_x = np.subtract(self.waypoints[goal_index][0] , self.waypoints[goal_index -1][0])
            delta_y = np.subtract(self.waypoints[goal_index][1] , self.waypoints[goal_index -1][1])
        ...
        # ------------------------------------------------------------------
        theta = -ego_state[2]
        goal_x = np.subtract(np.multiply(goal_state_local[0] , cos(theta)) , np.multiply(goal_state_local[1] , sin(theta)))
        goal_y = np.subtract(np.multiply(goal_state_local[0] , sin(theta)) , np.multiply(goal_state_local[1] , cos(theta)))
        # ------------------------------------------------------------------
        ...
        for i in range(self._num_paths):
            ...
            offset = np.multiply((i - self._num_paths // 2) , self._path_offset)
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            x_offset = np.multiply( offset , np.cos(goal_t + np.pi/2))
            y_offset = np.multiply(offset , np.sin(goal_t + np.pi/2))
            # ------------------------------------------------------------------
