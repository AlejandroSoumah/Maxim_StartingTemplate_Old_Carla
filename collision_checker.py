    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = np.add(path[0][j] , np.multiply(circle_offset ,  np.cos(path[2][j])))
                circle_locations[:, 1] = np.add(path[1][j] , np.multiply(circle_offset ,  np.sin(path[2][j])))
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                score = np.sqrt(np.add(np.square(np.subtract(paths[i][0][-1] , goal_state[0])) , np.square(np.subtract(paths[i][1][-1] , goal_state[1]))))
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            #score += self._weight *  1 /  ( np.sqrt( np.square(path[i][0] - path[collision_check_array[j]][0])) + np.square(path[i][1] - path[collision_check_array[i]][1])) 
                            # --------------------------------------------------
